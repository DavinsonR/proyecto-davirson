    ...
    <main>
      ...
      <header className="pt-20 pb-12">
        <div className={wrap}>
          ...
          <p className="text-[13.5px] text-body mt-3">{cv.metaLine}</p>
          <div className="mt-8 flex flex-wrap gap-3.5">
            <a href={cv.downloadHref} className="text-[13px] px-5 py-3 rounded-[3px] bg-cold text-paper font-semibold hover:opacity-90 transition-opacity">
              {cv.download}
            </a>
            <a href={`mailto:${dict.profile.email}`} className="text-[13px] px-5 py-3 rounded-[3px] border border-rule text-ink hover:border-cold transition-colors">
              {cv.contactBtn}
            </a>
          </div>
          {/* quick facts */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="border-t border-rule pt-5">
              ...
              <div className="flex flex-wrap gap-2">
                {cv.skillsFin.map((s) => (
                  <span key={s} className="rounded-[3px] border border-rule bg-band px-3 py-1.5 text-[13px] text-body">{s}</span>
                ))}
              </div>
            </div>
            <div className="border-t border-rule pt-5">
                ...
                {cv.skillsTech.map((s) => (
                  <div key={s.name}>
                    <div className="flex justify-between items-baseline gap-3">
                      ...
                      <span className="text-[10.5px] text-muted whitespace-nowrap">{s.note}</span>
                    </div>
            ...
            {cv.experience.map((company) => (
              <div key={company.company} className="border-t border-rule pt-5">
                ...
                {"note" in company && company.note ? (
                  <p className="text-[13px] text-cold mb-4">{company.note}</p>
                ) : (
                  <div className="mb-4" />
                )}
                  ...
                  {company.roles.map((role) => (
                    <div key={role.title} className="border-l border-rule pl-5">
                      <div className="flex flex-wrap items-baseline justify-between gap-2">
                        ...
                        <span className="text-[11.5px] text-muted">{role.period}</span>
                      </div>
      ...
      <section className="py-14 border-t border-rulesoft">
        <div className={wrap}>
          <div className="border-t-2 border-warm pt-6">
            <p className="text-[11px] tracking-[0.14em] uppercase text-warm mb-4">{cv.pivot.label}</p>
            ...
            {cv.awards.map((a) => (
              <div key={a.title} className="border-t border-rule pt-5">
                <div className="flex items-baseline justify-between gap-3">
                  ...
                  <span className="text-[11.5px] text-muted">{a.year}</span>
                </div>
            ...
            <div className="flex flex-col gap-4">
              {cv.education.map((e) => (
                <div key={e.title} className="border-t border-rule pt-5">
                  ...
                  <p className="text-[11.5px] text-muted mt-1">{e.period}</p>
                </div>
              ))}
            </div>
              ...
              {cv.certs.map((c, i) => (
                <div key={c.title} className={`py-3.5 ${i !== 0 ? "border-t border-rulesoft" : ""}`}>
                  <div className="flex items-baseline justify-between gap-3">
                    ...
                    <span className="text-[11.5px] text-muted">{c.year}</span>
                  </div>
      ...
      <section className="py-14 border-t border-rulesoft">
        <div className={wrap}>
          ...
          <div className="mt-10">
            <a href={`mailto:${dict.profile.email}`} className="text-[13px] px-5 py-3 rounded-[3px] bg-cold text-paper font-semibold hover:opacity-90 transition-opacity inline-block">
              {cv.contactBtn} →
            </a>
          </div>
        </div>
